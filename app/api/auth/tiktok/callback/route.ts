...
export async function GET(req: NextRequest) {
  ...
  type TikTokAuthShop = { id?: string; cipher?: string };
  const shops = (d.shops ?? []) as TikTokAuthShop[];
  const shopFromToken = shops.find((s) => s.id === shopId) ?? shops[0];
  let shopCipher = shopFromToken?.cipher ?? '';
  let resolvedShopId = shopFromToken?.id ?? shopId ?? '';
  ...
  console.log(
    '[TikTok callback] resolved shop_id:',
    resolvedShopId,
    '| cipher present:',
    !!shopCipher,
  );
