...
import type { TikTokOrderListItem, TikTokOrderDetail } from '@/types/tiktok';
...
export function mapStatus(tikTokStatus: string): string {
  ...
  while (start < to && allOrders.length < cap) {
    ...
    while (hasMore && allOrders.length < cap) {
      ...
      const data = await callTikTok<{
        ...
        next_cursor?: string;
        total_count?: number;
      }>('/order/202309/orders/search', queryParams, { method: 'POST', body });
...
export async function fetchTikTokOrderDetails(orderIds: string[]): Promise<TikTokOrderDetail[]> {
  ...
  for (let i = 0; i < orderIds.length; i += BATCH_SIZE) {
    ...
    const data = await callTikTok<{ orders: TikTokOrderDetail[] }>('/order/202309/orders', {
      ids: batch.join(','),
    });
 ...
 */
export function tikTokOrdersToRows(orders: TikTokOrderDetail[]): {
