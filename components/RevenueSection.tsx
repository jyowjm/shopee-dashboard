...
import type { RevenueData, Platform } from '@/types/dashboard';
import {
  getPrevPeriodLabel,
  DeltaBadge,
...
export default function RevenueSection({
  ...
  const { data, loading, error, retry } = useFetch<RevenueData>(async () => {
    ...
    if (platform === 'all' && hasShopee && hasTikTok) {
      const [s, t] = await Promise.all([
        fetchJson<RevenueData>(`/api/shopee/revenue?${qs}`),
        fetchJson<RevenueData>(`/api/tiktok/revenue?${qs}`),
      ]);
      return mergeRevenue(s, t);
    }
    const url = platform === 'tiktok' ? `/api/tiktok/revenue?${qs}` : `/api/shopee/revenue?${qs}`;
    return fetchJson<RevenueData>(url);
  ...
  if (error) return <SectionError title="Revenue" message={error} onRetry={retry} />;
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <>
              ...
              <div>
                <div className="flex items-start gap-2">
                  ...
                  {data && (
                    <DeltaBadge
                      current={data.order_count}
                      previous={data.prev_order_count}
                      periodLabel={periodLabel}
                    />
                  )}
                </div>
      ...
      <ResponsiveContainer width="100%" height={200}>
        <LineChart data={chartData}>
          ...
          <Tooltip
            formatter={(v: unknown) => [`RM ${(v as number).toFixed(2)}`, 'Revenue']}
            ...
            <table className="w-full text-sm mt-3">
              ...
              <tbody>
                {data!.orders.map((o) => (
                  <tr key={o.order_sn} className="border-b border-gray-50 last:border-0">
                    ...
                    <td
                      className={`py-2 text-xs font-medium ${STATUS_COLOR_CLASSES[o.status] ?? 'text-gray-500'}`}
                    >
              ...
              <tfoot>
                <tr className="border-t-2 border-gray-200">
                  <td colSpan={3} className="pt-2 text-sm font-semibold text-gray-700">
                    Total
                  </td>
