...
import type { CustomerData, Platform } from '@/types/dashboard';
...
function mergeCustomers(a: CustomerData, b: CustomerData): CustomerData {
  ...
  const allOrders =
    a.avg_orders_per_customer * a.total_unique_customers +
    b.avg_orders_per_customer * b.total_unique_customers;
  const allSpend =
    a.avg_spend_per_customer * a.total_unique_customers +
  ...
  return {
    ...
    repeat_purchase_rate:
      total > 0
        ? (((a.repeat_purchase_rate / 100) * a.total_unique_customers +
            (b.repeat_purchase_rate / 100) * b.total_unique_customers) /
            total) *
          100
        : 0,
    ...
    location_coverage: {
      orders_with_state:
        a.location_coverage.orders_with_state + b.location_coverage.orders_with_state,
      total_paid_orders:
        a.location_coverage.total_paid_orders + b.location_coverage.total_paid_orders,
    },
  };
}
...
export default function CustomersSection({
  ...
  const { data, loading, error, retry } = useFetch<CustomerData>(async () => {
    ...
    if (platform === 'all' && hasShopee && hasTikTok) {
      const [s, t] = await Promise.all([
        fetchJson<CustomerData>(`/api/shopee/customers?${qs}`),
        fetchJson<CustomerData>(`/api/tiktok/customers?${qs}`),
      ]);
      return mergeCustomers(s, t);
    }
    const url =
      platform === 'tiktok' ? `/api/tiktok/customers?${qs}` : `/api/shopee/customers?${qs}`;
    return fetchJson<CustomerData>(url);
  ...
  if (error) return <SectionError title="Customers" message={error} onRetry={retry} />;
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
        ...
        <div className="p-4 bg-gray-50 rounded-lg">
          ...
          <p className="text-2xl font-bold text-gray-900">
            {(data?.avg_orders_per_customer ?? 0).toFixed(1)}
          </p>
        </div>
        <div className="p-4 bg-gray-50 rounded-lg">
          <p className="text-xs text-gray-500 mb-1">Avg Spend / Customer</p>
          <p className="text-2xl font-bold text-gray-900">
            RM {(data?.avg_spend_per_customer ?? 0).toFixed(2)}
          </p>
        </div>
      </div>
          ...
          <div className="flex items-baseline gap-2 mb-3">
            ...
            {data.location_coverage && data.location_coverage.total_paid_orders > 0 && (
              <p className="text-xs text-gray-400">
                {data.location_coverage.orders_with_state} of{' '}
                {data.location_coverage.total_paid_orders} paid orders have location data
              </p>
            )}
          </div>
